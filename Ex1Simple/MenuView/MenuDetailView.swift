...
struct MenuDetailView: View {
    ...
    @State var itemOrdered:Bool = false
...
//        orderModel.add(menuID: menuItem.id)
    ...
    var body: some View {
        VStack {
            ...
            SelectedImageView(imgName: "\(menuItem.id)_250w")
                .padding(5)
                .layoutPriority(3)
...
//                .alert(isPresented: $itemOrdered) {
//                    Alert(title: Text("Order"), message: Text("You ordered a \(menuItem.name)"))}
                .sheet(isPresented: $itemOrdered) {
                    ConfirmView(menuID: self.menuItem.id, quantity: $quantity, size: $settings.size, isPresented: $itemOrdered, orderModel: self.orderModel)
                }
                Spacer()
            }
            .padding(.top)
...
struct QtyStepperView: View {
    @Binding var quantity:Int
    var body: some View {
        Stepper(value: $quantity, in: 1...10) {
            Text("Quentity: \(quantity)")
                .bold()
        }
        .padding()
    }
